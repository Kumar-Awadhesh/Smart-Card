import { useState } from "react"

const NewDrivingLicence = () => {



    return(
        <>
            <main className="licence-container">
                <div className="form-container">
                    <div className="front-container">
                        <div>
                            <h4>Driving Licence Number :</h4>
                            <input type="text" placeholder="Enter Driving Licence Number" />
                        </div>
                        <div>
                            <h4>Licence Issue Date :</h4>
                            <input type="date" placeholder="Select issue date"/>
                        </div>
                        <div>
                            <h4> Licence Validity :</h4>
                            <input type="date" placeholder="Select Validity date" />
                        </div>
                        <div>
                            <h4>Holder's Photo :</h4>
                            <input type="file" placeholder="Upload photo" />
                        </div>
                        <div>
                            <h4>Holder's Signature :</h4>
                            <input type="file" placeholder="Upload signature" />
                        </div>
                        <div>
                            <h4>Holder's Name :</h4>
                            <input type="text" placeholder="Enter your name" />
                        </div>
                        <div>
                            <h4>Date of Birth :</h4>
                            <input type="date" placeholder="Select your date of birth" />
                        </div>
                        <div>
                            <h4>Father's Name :</h4>
                            <input type="text" placeholder="Enter your father's name" />
                        </div>
                        <div>
                            <h4>Address :</h4>
                            <textarea name="" id="" placeholder="Enter your address"></textarea>
                        </div>
                    </div>
                    <div className="back-container">
                        <div>
                            <h4>Vehicle Code :</h4>
                            <input type="text" placeholder="Enter vehicle code" />
                        </div>
                        <div>
                            <h4>Mobile Number :</h4>
                            <input type="number" placeholder="Enter your mobile number" />
                        </div>
                        <div>
                            <h4>Licence Authority :</h4>
                            <input type="text" placeholder="Enter licence authority name" />
                        </div>
                    </div>
                    <div className="submit-btn-container"><button>Submit</button></div>
                </div>
            </main>
        </>
    )
}

export default NewDrivingLicence;